...
async function scrape() {
  ...
  const tableData = await page.evaluate(() => {
    // A helper function we will use below remove all non-asci characters and all newlines from a string
    ...
    return tables.map(table => {
      ...
      return rows.slice(1).map(row => {
        ...
        // Use regex to remove all non ascii characters from the string, then to
